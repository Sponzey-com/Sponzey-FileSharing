...
- UDP는 낮은 지연의 discovery/control에 사용하고, 파일 payload 전송은 인증된 TCP Data Channel을 사용합니다.
- TCP stream의 순서 보장, 재전송, 흐름 제어를 활용하되 application frame, digest 검증, 취소, 실패 복구는 앱 계층에서 명시적으로 관리합니다.
...
### UDP Discovery/Control과 TCP Data 전송

Discovery와 control은 UDP 기반을 유지합니다. Broadcast discovery와 낮은 지연의 로컬 control message는 peer 탐색과 세션 협상에 적합하기 때문입니다. 파일 payload 전송은 인증된 TCP Data Channel을 사용해 UDP source port 변화, route candidate 흔들림, discovery lease 만료가 전송 중 active peer session을 무효화하지 않도록 합니다.

TCP Data Channel 계층은 다음 문제를 명시적으로 다룹니다.

- stream frame 경계
- transfer metadata 협상
- backpressure와 flow control
- 전송 취소
- socket close와 reconnect 판단
- 부분 실패와 재시도
- 수신 완료 후 digest 검증
...
- Flutter Desktop 앱
- 로컬 네트워크 노드 탐색
- UDP 기반 discovery/control과 TCP Data Channel 파일 payload 전송
- 가입 없는 런타임 ID/PW 세션
- 인증된 peer 연결
- 단일 파일 및 다중 파일 전송
- 1:1 및 1:N 전송 구조
...
- Riverpod
- Drift / SQLite
- UDP discovery/control socket과 TCP Data Channel stream transport
...
lib/
  ...
  infrastructure/      UDP/TCP, 인증, DB, 파일 시스템, 플랫폼 구현
...
- `domain`은 Flutter, Riverpod, Drift, UDP/TCP socket, 파일 시스템에 의존하지 않습니다.
...
Sponzey FileSharing은 현재 TCP Data Channel 계획에서 discovery/control에는 UDP를 사용하고, 파일 payload 전송에는 TCP를 사용합니다. 플랫폼 문제는 OS별 프로토콜 분기가 아니라 플랫폼 경계에서 해결해야 합니다.
...
- TCP listener는 인증된 control 협상 이후 결정됩니다.
- `AppConfig`가 bootstrap 시점에 data listener port를 명시적으로 고정하지 않는 한 OS가 배정한 ephemeral TCP port를 우선합니다.
- 고정 TCP Data Channel port를 도입하면 같은 변경에서 방화벽 안내와 smoke test 기준도 함께 갱신해야 합니다.
...
- discovery가 동작하지 않으면 설정된 UDP 포트를 명시적으로 허용합니다. 기본값은 discovery `38400/udp`, control `38401/udp`입니다.
- peer 인증 후 파일 전송이 동작하지 않으면 Private 네트워크에서 앱의 inbound TCP를 허용하거나, bootstrap에서 고정 TCP Data Channel port를 설정한 경우 해당 TCP 포트를 허용합니다.
...
5. 인증된 연결이 연결된 TCP Data Channel까지 도달하고 route candidate는 진단 컨텍스트로만 남는지 확인합니다.
6. TCP Data Channel을 통해 작은 파일을 양방향으로 전송합니다.
7. 수신 파일이 기본 수신 경로 아래에 생성되는지 확인합니다.
8. diagnostics export를 생성해 route, auth, TCP data session, transfer, storage 상태가 포함되고 password, JWT, session key, file payload, 민감한 전체 경로가 포함되지 않는지 확인합니다.
...
- [.tasks/plan.md](.tasks/plan.md): 현재 TCP Data Channel 전환 계획
...
작업 태스크는 `.tasks`와 phase archive 디렉터리 아래에 정리되어 있습니다. 현재 TCP Data Channel 전환 계획은 `.tasks/plan.md`에 두고, 현재 실행 태스크는 `.tasks/task001.md`부터 한 번에 하나씩 생성합니다.

## 현재 개발 흐름

현재 TCP Data Channel 전환 흐름은 `.tasks/plan.md`를 기준으로 진행됩니다.

1. 제품 문서와 태스크 기준 정렬
2. TCP data peer session state machine과 값 객체 추가
3. DataChannel abstraction을 legacy UDP data transfer code와 분리
4. TCP listener, connector, 인증된 control negotiation 추가
5. TCP data session을 인증된 peer session에 binding
6. TCP frame으로 파일을 streaming하고 digest 검증과 취소 처리
7. TCP 기반 queue, 양방향 전송, multi-peer isolation 지원
8. TCP data 상태를 UI와 diagnostics에 노출하되 transport 세부사항은 숨김
9. UDP Data 기본 경로를 격리하거나 제거
10. macOS, Windows, Linux platform behavior hardening
11. 양방향 host/VM TCP Data Channel 검증을 포함한 release gate 적용
...
Sponzey FileSharing은 로컬 네트워크 안에서 빠르고 안전하게 파일을 주고받기 위한 데스크톱 앱입니다. UDP는 discovery/control에 사용하고, 인증된 TCP Data Channel은 파일 payload 전송에 사용하며, 인증 경계, 상태 머신 기반 절차 관리, MessageBus 기반 이벤트 전달, 테스트 가능한 계층 구조를 통해 실사용 가능한 내부망 파일 전송 도구로 발전시키는 것을 목표로 합니다.
