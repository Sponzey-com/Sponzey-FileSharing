...
- 사용자 계정 생성 및 로그인 후 인증된 세션을 열 수 있다.
...
- 사용자는 앱 최초 실행 시 로컬 계정을 생성할 수 있어야 한다.
- 계정은 `ID`, `표시 이름`, `비밀번호`, `장치 이름`을 가진다.
- 비밀번호는 평문 저장 금지다.
...
- 사용자는 상대 노드에 연결 요청을 보낼 수 있어야 한다.
- 상대 노드는 요청자를 인증하고 수락/거절할 수 있어야 한다.
...
- 수신자는 저장 경로를 정책에 따라 자동 결정하거나 수동 승인할 수 있어야 한다.
...
5. 수신자가 저장된 peer verifier 또는 공유된 credential metadata로 JWT를 검증한다.
...
완전한 중앙 인증 서버 없이 동작해야 하므로, 각 노드는 로컬 계정 저장소와 허용 사용자 목록, 그리고 허용된 상대에 대한 peer credential verifier를 관리한다.

즉:

- 각 앱은 자기 사용자 계정으로 로그인한다.
- 각 수신 노드는 "어떤 사용자 ID를 허용할지" 로컬 정책으로 가진다.
- 허용된 사용자만 파일을 보낼 수 있다.
- 각 수신 노드는 허용한 사용자 ID에 대해 JWT 검증에 필요한 verifier 또는 password-derived shared secret metadata를 가진다.
...
- `Argon2id` 해시 저장 권장
- 솔트는 사용자별 랜덤 값 사용
- 해시 파라미터는 추후 상향 가능하도록 메타데이터와 함께 저장
...
각 데스크톱 앱은 자기 로컬 사용자 계정을 가진다.

예시:

- 사용자 A가 자신의 PC에서 로그인
- 사용자 B의 PC를 탐색
- B는 로컬 정책상 A를 허용한 경우에만 수신

추가 전제:

- A가 B에게 인증받으려면 B는 A의 `user_id`에 대응하는 peer credential verifier를 로컬에 가지고 있어야 한다.
- 반대로 상호 인증을 강제하면 A도 B의 verifier를 가지고 있어야 한다.
- 즉, 이 구조는 완전한 공개 인증이 아니라 "허용된 사용자 간의 사전 등록형 인증"에 가깝다.

## 9.2 권한 정책

수신 노드는 최소 다음 정책을 가진다.
...
- 수신자가 저장된 peer verifier 또는 공유된 derived credential로 JWT 서명과 claim 검증
...
- DB: 일반 메타데이터
- `flutter_secure_storage`: 앱 비밀키, 세션 관련 영속 키, 암호화용 로컬 마스터 키

Linux에서는 Secret Service 의존성이 있을 수 있으므로 배포 문서에 이를 명시해야 한다.
...
- `Riverpod` 기반 상태 관리
- `SQLite + drift` 로컬 저장소
- `flutter_secure_storage` 기반 민감 정보 저장
