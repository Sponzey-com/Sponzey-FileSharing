...
class _TransfersScreenState extends ConsumerState<TransfersScreen> {
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        initialValue:
                            _selectedPeerId != null &&
                                peers.any((peer) => peer.id == _selectedPeerId)
                            ? _selectedPeerId
                            : null,
                        ...
                        child: AnimatedContainer(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'MVP에서는 단일 파일만 전송합니다. 여러 파일을 드롭하면 첫 파일만 사용합니다.',
                                style: Theme.of(context).textTheme.bodyMedium,
                              ),
                              if (_filePathController.text
                                  .trim()
                                  .isNotEmpty) ...[
                                const SizedBox(height: AppSpacing.sm),
                                Text(
                                  '선택됨: ${p.basename(_filePathController.text.trim())}',
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                  style: Theme.of(context).textTheme.bodyMedium,
                                ),
  ...
  Future<void> _applyDroppedFiles(List<XFile> files) async {
    ...
    final firstPath = files.first.path.trim();
    if (firstPath.isEmpty) {
      return;
    }

    if (!FileSystemEntity.isFileSync(firstPath)) {
      if (!mounted) {
        return;
      }
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    setState(() {
      _filePathController.text = firstPath;
    });

    if (files.length > 1 && mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('여러 파일 중 첫 번째 파일만 선택했습니다.')));
    }
  }
}
