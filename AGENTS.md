...
- peer identity와 route identity를 분리한다. 하나의 peer는 여러 route candidate를 가질 수 있으며, UI 기본 표시와 전송 대상 선택은 검증된 active route lease를 통해 안정화한다.
- route lease에는 최소한 local interface, local address, remote address, control endpoint, data endpoint, 검증 시각, 만료 정책이 포함되어야 한다. 파일 전송은 검증되지 않은 route candidate로 시작하지 않는다.
